...
public class AuditService {
    ...
    public void logRegistration(String username, String email, String role, boolean success, String reason) {
        String message = String.format(
                "[AUDIT] Registration | Username: %s | Email: %s | Role: %s | Success: %s | Reason: %s | Timestamp: %s",
                username, email, role, success, reason, LocalDateTime.now().format(formatter)
        );
        if (success) {
            logger.info(message);
        } else {
